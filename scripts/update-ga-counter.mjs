...
function parseConfigGoogleAnalyticsId(configText) {
  const match = configText.match(/^\s*google_analytics:\s*["']?([^"'#\r\n]+)["']?/m);
...
async function fetchAccessToken(assertion) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch Google OAuth token (${response.status}): ${detail}`);
  }
...
async function fetchGaCounter({ propertyId, accessToken }) {
  ...
  if (!response.ok) {
    const detail = await response.text();
    throw new Error(`Failed to fetch GA4 report (${response.status}): ${detail}`);
  }

  const payload = await response.json();
  const metricValues = payload?.rows?.[0]?.metricValues;
  if (!Array.isArray(metricValues) || metricValues.length < 2) {
    throw new Error('Unexpected GA4 report response: metric values are missing.');
  }
  ...
  if (!Number.isFinite(pageViews) || !Number.isFinite(users)) {
    throw new Error('Unexpected GA4 report response: metric values are not numeric.');
  }
...
async function main() {
  ...
  const { pageViews, users } = await fetchGaCounter({ propertyId, accessToken });
  ...
  console.log(`Updated ${OUTPUT_PATH}: PV=${payload.page_views_30d_text}, Users=${payload.users_30d_text}`);
}
