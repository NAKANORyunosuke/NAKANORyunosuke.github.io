...
## プロジェクト概要
Twitchのサブスク限定Discordサーバー向けに, サブスク認証とロール/チャンネル管理を全自動化した.   
視聴者は `/link` コマンドからOAuth認可を行うだけでロール付与が完了し, EventSub(Webhook)により再サブや解約も即時反映される. 

- **目的**：手動でのサブスク確認とロール付与をゼロにし, 運用負荷とミスを排除すること  
- **成果**：サブスクTier別ロール/チャンネルの自動整備, 再サブと解約の検知, 月次再リンクDMと未解決再送, 参加時の自動DM, HTTPS対応までを実現
...
## 運用とセキュリティ
- HSTSとHTTPSリダイレクトを導入して安全なOAuthフローを提供  
