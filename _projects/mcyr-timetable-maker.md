...
## 概要
MCYR Timetable Maker は、学会や研究会向けの「講演データ統合 + タイムテーブル編成 + 配布物出力」を一気通貫で扱う業務システムです。
編成UIにとどまらず、以下を1サービス内で完結させています。
...
- 編成後の配布資料（一覧PDFと講究録PDF）作成が別作業で重かった
...
- 講究録PDFをCeleryジョブ化し、進捗表示、再利用、失敗時リカバリを実装
...
- `/operations/master-edit/` マスタ修正（講演者と投稿の編集、講演者統合）
- `/operations/reconciliation/` 照合課題解決
- `/wiki/` `/wiki/user/` `/wiki/dev/` 運用および開発向けWiki
